...
- Photos and raster images: pull the exact assets from the Figma frame, **rename each to a
  ...
  method; it serves WebP and keeps alt but lacks `srcset`, which a human restores by binding in the
  builder). Never use an external Figma URL. **Read names, section labels and content from the design to name
